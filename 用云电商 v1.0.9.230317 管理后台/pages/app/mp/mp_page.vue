<template>
	<div>
		...
		<div class="container padding_b_0">
			<div class="dflex_wrap">
				<div class="dflex_vertical_c margin_r_40 margin_b_20">
					<div class="search_name">页面名称：</div>
					<el-input v-model="req.name" placeholder="请输入" class="search_input" @input="loadData"></el-input>
				</div>
				<div class="dflex_vertical_c margin_r_40 margin_b_20">
					<div class="search_name">URL地址：</div>
					<el-input v-model="req.url" placeholder="请输入" class="search_input" @input="loadData"></el-input>
				</div>
				<div class="dflex_vertical_c margin_r_40 margin_b_20" v-if="advancedSearch">
					<div class="search_name">页面类型：</div>
		...
		<div class="container use-table">
			...
			<el-table :height="tblHeight" :data="tableDatas" @selection-change="handleSelectionChange"
				...
				<el-table-column property="name" label="页面名称" align="center"></el-table-column>
				<el-table-column property="type" label="页面类型" align="center"></el-table-column>
				<el-table-column property="url" label="url地址" align="center"></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toAdd() {
				uni.navigateTo({
					url: `/pages/app/mp/mp_page_add_edit?tab=添加小程序`,
					events: {
						refreshData: () => {
							this.loadData();
						}
			...
			toEdit(id) {
				uni.navigateTo({
					url: `/pages/app/mp/mp_page_add_edit?id=${id}&tab=编辑小程序`,
					events: {
						refreshData: () => {
							this.loadData();
						}
