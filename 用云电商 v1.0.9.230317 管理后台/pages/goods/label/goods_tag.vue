		...
		<div class="container use-table">
			...
			<el-table :height="tblHeight" :data="tableDatas" @selection-change="selectionChange" highlight-current-row>
				...
				<el-table-column label="图标" align="center">
					<template slot-scope="scope">
						<el-image style="width: 100px;height: 80px;" :src="scope.row.img"
							:preview-src-list="scope.row.imgs" fit="contain"></el-image>
					</template>
				</el-table-column>
				<el-table-column label="分类搜索" align="center">
					<template slot-scope="scope">
						<el-tooltip :content="scope.row.is_search == '否' ? '分类搜索' : '取消分类搜索'" placement="top"
							:hide-after="1000" :enterable="false" effect="light">
							<el-switch v-model="scope.row.is_search" active-color="#ff6a6c" inactive-color="#bbb"
								active-value="是" inactive-value="否" @change="classChange(scope.row)"></el-switch>
						</el-tooltip>
					</template>
				</el-table-column>
